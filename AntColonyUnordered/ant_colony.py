...
class AntTSP:
	...
	def __init__(self, distance):
		...
		self.best_ant_result = sys.maxsize
		self.best_ant_path = []

		self.pheromone = [[self.PHEROMONE for x in range(self.ccount)] for y in range(self.ccount)]
	...
	def antInit(self, option_reset = False):
		for iant in range(self.acount):
			...
			print(option_reset)
			if (option_reset):
				print(Fore.GREEN + str(ant.traversed_length))
	...
	def movingAnts(self):
		...
		for iant in range(self.acount):
			ant = self.ants[iant]
			if (ant.path_index < self.ccount):	# пртому что мы проходим все города и не возвращаемся обратно. Для этого мы вручную вписали условие
				moving += 1
				...
				if (ant.path_index == self.ccount):
					...
					ant.traversed_length += self.distance[ant.path[len(ant.path)-1]][ant.path[0]]	# -2, а не -1 потому что у мы не возврщаемся в начальный город
					ant.path_index += 1
	...
	def run(self, epochs):
		...
		return self.best_ant_path, self.best_ant_result
